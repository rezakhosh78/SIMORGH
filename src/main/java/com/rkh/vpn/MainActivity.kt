...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            val liveQrScanner = rememberLauncherForActivityResult(ScanContract()) { result ->
                val text = result.contents
                if (!text.isNullOrBlank()) {
                    vm.importScannedText(text)
                    vm.loadSubscription()
                } else {
                    vm.updateStatus("QR scan cancelled")
                }
            }
            ...
            val cameraPermission = rememberLauncherForActivityResult(
                ...
                if (granted) {
                    liveQrScanner.launch(qrOptions())
                } else {
                    vm.updateStatus("Camera permission denied")
                }
            }
    ...
    private fun qrOptions(): ScanOptions = ScanOptions().apply {
        setDesiredBarcodeFormats(ScanOptions.QR_CODE)
        setPrompt("Scan SIMORGH private subscription QR")
        setBeepEnabled(true)
