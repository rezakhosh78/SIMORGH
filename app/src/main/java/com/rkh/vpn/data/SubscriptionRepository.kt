...
class SubscriptionRepository {
    companion object {
        // Legacy private subscription endpoints were removed from the app binary.
        // These values are only local mode identifiers for the old Standard/Premium UI.
        const val PRIMARY_BASE = "simorgh://subscription/standard/"
        const val PREMIUM_BASE = "simorgh://subscription/premium/"
    }
    ...
    fun fetch(base: String, token: String): Result {
        ...
        if (!base.startsWith("http://") && !base.startsWith("https://")) {
            throw IllegalStateException("Subscription endpoint is not configured")
        }
